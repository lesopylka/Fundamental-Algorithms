...
//как же я люблю фибоначчиву кучу ох моя фибоначчава куча тащусь от нее
...
//(поиск, сортировка, добавление/удаление, модификация и т. п.) и вывод данных в поток вывода.
#include <math.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
void insertion(FIB_HEAP *H, NODE *newNode, int val, struct Employee employee);
...
//  surname: lineArguments[1];
//  wage: lineArguments[2];
// }

// 1. считываешь строку ( по хорошему делать валидацию )
// 2. Вставляешь новый узел в кучу с помощью insertion(указатель на кучу, указатель на пустой узел, ключ(первый аргумент строки), структура employee (остальные три аргумента стркои))
...
void insertion(FIB_HEAP *H, NODE *newNode, int val, struct Employee employee) {
  ...
  if (H->min == NULL) {
    ...
    H->min->left_sibling->right_sibling = newNode;
    newNode->right_sibling = H->min;
    newNode->left_sibling = H->min->left_sibling;
    H->min->left_sibling = newNode;
...
void decrease_key(FIB_HEAP *H, NODE *node_to_be_decrease, int new_key) {
  ...
  if (node_to_be_decrease == NULL) {
    ...
    if (node_to_be_decrease->key < new_key) {
      printf("\n Invalid newNode key for decrease key operation \n ");
    } else {
...
FIB_HEAP *insertion_procedure() {
  ...
  for (i = 1; i <= no_of_nodes; i++) {
    ...
    scanf("%d", &ele);
    //считать employee
    insertion(temp, new_node, ele, employee);
  }
  return temp;
}
